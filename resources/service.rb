...
action :create do
  ...
  systemd_service_names = [new_resource.service_name]
  systemd_service_names << 'named' if platform?('debian') && new_resource.service_name == 'bind9'

  systemd_service_settings = [
    '# Generated by Chef',
    '# Workaround upstream issue as described in https://bugs.debian.org/983216',
    '[Service]',
  ]
  systemd_service_settings << 'Type=simple' if platform?('debian') && new_resource.chroot
  ...
  systemd_service_names.uniq.each do |systemd_service_name|
    directory "/etc/systemd/system/#{systemd_service_name}.service.d"

    file "/etc/systemd/system/#{systemd_service_name}.service.d/managed-keys.conf" do
      content "#{systemd_service_settings.join("\n")}\n"
