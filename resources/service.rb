...
action :create do
  ...
  if systemd?
    ...
    directory '/etc/systemd/system/named.service.d'

    file '/etc/systemd/system/named.service.d/managed-keys.conf' do
      content <<~EOU
        # Generated by Chef
        # Workaround upstream issue as described in https://bugs.debian.org/983216
        [Service]
        ExecCondition=/bin/rm -fv #{new_resource.vardir}/managed-keys.bind
        ExecCondition=/bin/rm -fv #{new_resource.vardir}/managed-keys.bind.jnl
      EOU
